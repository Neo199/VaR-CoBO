# SIMULATION STUDY
# MAXSAT Problem from Deshwal Paper
...
maxsat_obj <- read_maxsat("/Users/niyati/Projects:Codes/PhD-Compute/R code/TestPrblms/MaxSAT/maxsat.wcnf")

model <- function(x_vals, seed){
  maxsat(x_vals, maxsat_obj, seed) 
}
...
run_simulation_instance <- function(instance_id, n_vars, evalBudget, n_init, order,
  ...
  tryCatch({
    ...
    y_vals <- maxsat(x_vals, maxsat_obj, seed) 
...
num_cores <- 1  # Use all cores minus one to avoid overloading your system
