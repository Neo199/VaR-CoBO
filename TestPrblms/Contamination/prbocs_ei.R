# SIMULATION STUDY
# Function for PRBOCS-EI-optim

prbocs_ei <- function(data, evalBudget, n_iter, n_vars, xTrain, xTrain_in, theta_current, order){
  # Initialize a data frame to store iteration results
  prbocsei_result <- matrix(0, evalBudget, n_vars)
  
  #Initialise BOCS-GA specific dataframe
  prbocsei_data <- data
  f_best <- min(prbocsei_data$y)
  ...
  for (t in 1:n_iter) {
    ...
    print(paste("prbocsei_iteration_",t))
    ...
    prbocsei_data <- rbind(prbocsei_data, data_new)
    f_best <- min(prbocsei_data$y)
  ...
  result <- list(solution = tail(prbocsei_data, n =1), data = prbocsei_data, 
                 model = prbocsei_bayesian_model) 
  return(result)
}
