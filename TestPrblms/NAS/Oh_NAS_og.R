...
make_nas_cell <- function(node_type, adj_mat, n_channels) {
  ...
  for (i in 2:(n_nodes-1)) {
    ...
    if (t1 == 0 && t2 == 0) {
      ...
      nodes[[i]] <- nn_conv2d(n_channels, n_channels, kernel_size = 3, padding = 1)
    } else {
      nodes[[i]] <- nn_conv2d(n_channels, n_channels, kernel_size = 5, padding = 2)
    }
  }
...
forward_nas_cell <- function(cell, x) {
  ...
  for (idx_set in cell$topo_order[-1]) {
    for (j in idx_set) {
      ...
      node_in <- if (length(inputs) == 0) torch_zeros_like(node_out[[1]]) else Reduce(`+`, inputs)
...
# NAS CNN Module
# ----------------------------
NASBinaryCNNModule <- nn_module(
  "NASBinaryCNNModule",
  initialize = function(model_list) {
    for (nm in names(model_list)) {
      if (inherits(model_list[[nm]], "nn_module")) self[[nm]] <- model_list[[nm]]
    }
    self$model_list <- model_list
  },
  forward = function(x) {
    forward_nas_cnn(self$model_list, x)
  }
)
...
build_nas_model <- function(x, data_type) {
  ...
  if (sum(adj[1,]) == 0 || sum(adj[,nrow(adj)]) == 0) return(NULL)
  adj <- valid_net_topo(adj)
  if (is.null(adj)) return(NULL)
  
  node_type <- x[22:31]
  model_list <- make_nas_cnn(data_type, node_type, adj,
                             info$n_ch_in, info$h_in, info$w_in, info$n_ch_base)
  
  NASBinaryCNNModule(model_list)
}
...
train_model <- function(model, n_epochs, train_loader, valid_loader, device = 0, verbose = TRUE) {
  ...
  best_acc <- 0
  dev <- torch_device(if (device == 0) "cpu" else paste0("cuda:", device-1))
  ...
  for (epoch in 1:n_epochs) {
    ...
    repeat {
      ...
      inputs <- batch[[1]]$to(device = device)   # <-- use 'device' not 'dev'
      labels <- batch[[2]]$to(device = device)   # <-- use 'device' not 'dev'
...
# Count parameters
# ----------------------------
count_ops <- function(model) sum(sapply(model$parameters, function(p) p$numel()))

# ----------------------------
# NAS Objective
# ----------------------------
nas_binary_evaluate <- function(x, data_type, loaders,
...
# ----------------------------
make_nas_objective <- function(data_type = "MNIST", batch_size = 32,
                               n_epochs = 5, device = 0, flop_weight = 0.02, seed = NULL,
                               verbose_each = TRUE) {
  loaders <- load_dataset(data_type, batch_size)
  objective <- function(x) {
    nas_binary_evaluate(x, data_type, loaders, n_epochs, device, flop_weight, seed, verbose_each)
  }
